...
arr = os.listdir('./extracted/dir_001')
...
for movieName in arr:
	...
	with open('./extracted/dir_001/'+movieName) as f:
		...
		for sentences in range(0, len(lines)):
			try:
				...
				output = nlp.annotate(lines[sentences], properties={'annotators': 'tokenize,ssplit,pos,depparse,parse', 'pipelineLanguage': 'zh','outputFormat': 'json'})
